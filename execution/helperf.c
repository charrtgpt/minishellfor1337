...
char	*get_command_path(char *cmd, char **envp, char **args, t_tofree *fr)
{
	if (ft_strchr(cmd, '/'))
	{
	...
	return (get_right_path(cmd, envp, args, fr));
}

void	execute_command(char *cmd_p, char **argv, char **envp, t_tofree *fr)
{
	struct stat	st;
